...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','fuwuleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','fuwuleixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('fuwuleixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussfuwuleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('fuwuleixing', this.detail);
				this.$utils.msg('审核成功');
