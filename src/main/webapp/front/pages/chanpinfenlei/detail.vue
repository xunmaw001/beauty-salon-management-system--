...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','chanpinfenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','chanpinfenlei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('chanpinfenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusschanpinfenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('chanpinfenlei', this.detail);
				this.$utils.msg('审核成功');
