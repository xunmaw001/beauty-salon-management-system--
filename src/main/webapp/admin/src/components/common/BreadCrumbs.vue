<template>
  <el-breadcrumb class="app-breadcrumb" separator=">>" style="height:40px;backgroundColor:rgba(117, 103, 168, 1);borderRadius:10px;padding:0px 20px 0px 20px;boxShadow:0px 0px 1px #91ae71;borderWidth:0 0 7px 0;borderStyle:groove ;borderColor:rgba(164, 120, 165, 1);">
    <transition-group name="breadcrumb" class="box" :style="2==1?'justifyContent:flex-start;':2==2?'justifyContent:center;':'justifyContent:flex-end;'">
      <el-breadcrumb-item v-for="(item,index) in levelList" :key="item.path">
        <span v-if="item.redirect==='noRedirect'||index==levelList.length-1" class="no-redirect" style="color:rgba(229, 235, 241, 0.98)">{{ item.name }}</span>
    ...
    breadcrumbStyleChange(val) {
      this.$nextTick(()=>{
        document.querySelectorAll('.app-breadcrumb .el-breadcrumb__separator').forEach(el=>{
          el.innerText = ">>"
          el.style.color = "rgba(214, 214, 214, 1)"
        })
        ...
        document.querySelectorAll('.app-breadcrumb .el-breadcrumb__inner .no-redirect').forEach(el=>{
          el.style.color = "rgba(229, 235, 241, 0.98)"
        })
